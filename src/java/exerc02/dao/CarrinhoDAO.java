...
public class CarrinhoDAO {
    ...
    public void encerraCarrinho(Carrinho carrinho) {
        try {
            sql = "update public.carrinho set ativo = false where public.carrinho.pkcarrinho = " + carrinho.getPkcarrinho() + ";";
            conecta.executarSQL(sql);
        } catch (ClassNotFoundException | IOException | SQLException ex) {
            Logger.getLogger(CarrinhoBean.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public ArrayList<CarrinhoItens> listarItens(Carrinho carrinho) {
        ...
        try {
            sql = "select * from public.carrinhoitens where public.carrinhoitens.fkcarrinho = " + carrinho.getPkcarrinho() + ";";
            rs = conecta.getDados(sql);

            while (rs.next()) {
                CarrinhoItens item = new CarrinhoItens();
                item.setFkcarrinho(rs.getInt("fkcarrinho"));
                item.setProduto(produtoDAO.obterPorID(rs.getInt("fkproduto")));
    ...
    public void inserirItem(Carrinho carrinho, CarrinhoItens item) {
        try {
            sql = "INSERT INTO public.carrinhoitens (\"fkcarrinho\",\"fkproduto\",\"quantidade\", \"total\")VALUES (" + carrinho.getPkcarrinho() + "," + item.getProduto().getId() + "," + item.getQuantidade() + "," + item.getValortotal() + ")";
            conecta.executarSQL(sql);
    ...
    public void atualizaItem(Carrinho carrinho, CarrinhoItens item) {
        try {
            sql = "update public.carrinhoitens set quantidade = " + item.getQuantidade() + ", total=" + item.getValortotal() + " where public.carrinhoitens.pkitem = " + item.getPkitem() + ";";
            conecta.executarSQL(sql);
            atualizaValor(carrinho);
        } catch (ClassNotFoundException | IOException | SQLException ex) {
    ...
    public void atualizaValor(Carrinho carrinho) {
        try {
            ...
            sql = "update public.carrinho set valortotal = " + valor + " where public.carrinho.pkcarrinho = " + carrinho.getPkcarrinho() + ";";
            conecta.executarSQL(sql);
        } catch (ClassNotFoundException | IOException | SQLException ex) {
            Logger.getLogger(CarrinhoDAO.class.getName()).log(Level.SEVERE, null, ex);
        }
